...
export function NativeLikesCounter({
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    try {
      await onLoadMore()
      // If we are passing down new users via props, we don't need local state for it.
      // But we can keep track of canLoadMore internally if we still want.
      // E.g. we might want the parent to control `hasMore` entirely.
    } catch (error) {
  ...
  const getVariantStyles = () => {
    ...
    switch (variant) {
      case "subtle":
        return cn(base, "bg-accent/50 hover:bg-accent", liked && "bg-accent")
      case "outline":
        return cn(
          base,
          "bg-transparent border border-border hover:border-accent-foreground/20 hover:bg-accent/10",
          liked && "border-accent-foreground/30 bg-accent/20",
        )
      case "ghost":
        return cn(base, "bg-transparent hover:bg-accent/50", liked && "bg-accent/30")
      default:
        return cn(
          base,
          "bg-accent border border-border hover:bg-accent/80 hover:border-accent-foreground/20",
          liked && "border-accent-foreground/20",
        )
    }
  }

  const visibleUsersInPopup = users.slice(0, maxVisibleInPopup)
  const totalRemaining = count - users.length
    ...
    <div className="relative inline-block" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      ...
      >
        ...
        <motion.div className="relative flex items-center justify-center">
          <motion.div animate={liked ? { scale: [1, 1.15, 1] } : { scale: 1 }} transition={{ duration: 0.2 }}>
            <Heart
              className={cn(
                sizeConfig.icon,
        ...
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            className={cn("font-medium tabular-nums", liked ? "text-foreground" : "text-muted-foreground")}
          >
      ...
      <AnimatePresence>
        {isHovered && users.length > 0 && (
          <motion.div
            ...
            className={cn(
              "absolute left-1/2 -translate-x-1/2 top-full mb-1 z-100",
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2 px-1">
              ...
              <span className="text-xs font-mono text-muted-foreground/60">{count.toLocaleString()}</span>
            </div>
