...
import { Star, ChevronRight, Package, Image as ImageIcon } from "lucide-react";
...
import { useTopAssetsAnalytics } from "@/features/dashboard-analytics/queries";

export function WidgetTopItems({
  className,
  tier,
  title,
}: {
  ...
  const { data: analyticsData, isLoading } = useTopAssetsAnalytics(tier);
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
        <div className="flex items-baseline gap-2 text-[13px]">
          <span className="text-cyber-body">Total downloads:</span>
          <span className="font-bold tabular-nums text-cyber-heading">
            4,381
          </span>
        </div>
        <div className="flex items-baseline gap-2 text-[13px]">
          <span className="text-cyber-body">Avg. Rating:</span>
          <span className="font-bold tabular-nums text-cyber-heading">4.8</span>
        </div>
      </div>
          ...
          items.map((item, idx) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-y-2 gap-x-4">
                {/* Left: Avatar & Text */}
                <div className="flex items-center gap-3 flex-1 min-w-[200px]">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-cyber-surface-active border border-cyber-border-subtle overflow-hidden">
                    {item.files && item.files[0] ? (
                      ...
                      <ImageIcon size={20} className="text-cyber-body" />
                    )}
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[13px] font-medium text-cyber-heading truncate max-w-[150px]">
                      {item.title}
                    </span>
                    <span className="text-[11px] text-cyber-body">
                ...
                <div className="flex-1 min-w-[120px] sm:text-center">
                  <span className="text-[12px] text-cyber-body tabular-nums">
                    {Number(item.totalDownloads).toLocaleString()} downloads
                  </span>
                </div>

                {/* Right: Badge (Fallback rating since not in DB) */}
                <div className="flex shrink-0 items-center justify-end">
                  <span className="inline-flex items-center gap-1 rounded-cyber bg-[rgba(0,230,118,0.08)] border border-[rgba(0,230,118,0.2)] px-2 py-1 text-[11px] font-medium text-neon-green tabular-nums">
                    ...
                    {((item.totalLikes / (item.totalViews || 1)) * 5).toFixed(
                      1,
                    )}
                  </span>
                </div>
