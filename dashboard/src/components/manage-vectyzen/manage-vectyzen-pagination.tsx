...
interface ManageVectyzenPaginationProps {
...
const ManageVectyzenPagination = ({
  queryState,
  setQueryState,
  meta,
}: ManageVectyzenPaginationProps) => {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-4 pt-4 border-t border-cyber-border">
      ...
      <div className="grid grid-cols-2 sm:flex sm:items-center gap-px rounded-cyber border border-cyber-border overflow-hidden bg-cyber-border">
        <button
          ...
          className="flex items-center justify-center gap-1 bg-cyber-surface px-3 py-2 text-[13px] font-medium text-cyber-body hover:bg-cyber-surface-hover hover:text-cyber-heading transition-colors disabled:opacity-50 disabled:hover:bg-cyber-surface"
        >
          <ChevronLeft size={16} />
          Previous
        </button>
...
export default ManageVectyzenPagination;
